            ...
            if (isset($check)) {
                ...
                if (!empty($check_date)) {
                    if ($check_date != $current_date) {
                        $property_id = UtilityPaid::select('property_id as property, utility_name')
                        ...
                        $input = [
                            ...
                            "user_id" => Auth::user()->id,
                            "property_id" => $property_id->property,
                            "description" => Auth::user()->name . " paid " . $upload['paid_by_user'] . " for " . $property_id->property_name,
                            'stt' => 1,
                            "stl" => 1
                        ];
                        $not = NotificationsController::insert($input);
                        return response()->json(["data" => [["utility_paid" => "added", "notification" => $not]]]);
                    }
                    ...
                    UtilityPaid::create($upload);
                    $utility_name = Utility::find($request->utility_id);
                    $input = [
                        'title' => "Utility Paid",
                        "user_id" => Auth::user()->id,
                        "property_id" => $property_id->property,
                        "description" => Auth::user()->name . " paid " . $upload['paid_by_user'] . " for " . $utility_name->utility_name,
                        'stt' => 1,
                        "stl" => 1
                    ];
                    $not = NotificationsController::insert($input);
    ...
    public function select(Request $request)
    {
        $select = UtilityPaid::join('utility', 'utility.id', '=', 'utility_paid.utility_id')
            ...
            ->select(
                ...
                'utility_paid.total_amount',
                'utility_paid.paid_by_user',
                'utility_paid.is_split'
            );
